...
We examine the use of a Frank copula to model dependency between two competing risks: firm-specific failure (a company failing on its own) and systemic failure (failure due to wider market or economic problems). The Frank copula provides a symmetric dependence structure, meaning that dependence is modelled evenly across the entire distribution — neither early nor late failures are disproportionately influential.

This setting reflects real-world scenarios where the two failure types may be influenced by shared but general macroeconomic or market factors, without a strong directional bias. For example, a company's own financial problems and wider market conditions might get worse at the same time, with both types of risk increasing together regardless of time. The Frank copula is ideal in such cases as it lacks tail dependence and instead models a more balanced, global dependency between variables.

The two failure times are drawn from gamma distributions, where both firm-specific and systemic risks are assigned the same marginal parameters to easily see patterns caused by their dependency: shape = 2 and rate = 4, corresponding to a mean failure time of 0.5 for each component. 
...
# failure time 
default_time <- pmin(T1, T2)
event_type <- ifelse(T1 < T2, 1, 2)

simulated_data <- data.frame(default_time, event_type)
#head(simulated_data)
...
## Visualise Default Times by Event Type

A histogram of default times coloured by event type shows the distribution of observed failures from firm-specific and systemic causes. Because both marginals are identically distributed, the differentiation between causes is driven entirely by the dependency structure introduced by the copula, where the distribution of causes is more balanced over time.

```{r}
# default times
ggplot(simulated_data, aes(x = default_time, fill = factor(event_type))) +
  geom_histogram(position = "identity", bins = 50, alpha = 0.6) +
  labs(title = "Histogram of Default Times", x = "Time", fill = "Event Type") +
  scale_fill_manual(values = c("steelblue", "tomato"),
                    labels = c("Firm-specific", "Systemic"))
...
## Extract and Fit Distributions to Each Failure Cause

Fitting the marginals for each failure cause using a selection of candidate distributions (exponential, weibull, gamma, and log-normal), the gamma distribution was correctly identified as the best fit in both cases. This allows us to move forward with parameter estimation under correct distributional assumptions.

```{r}
# firm and systemic
firm_specific <- subset(simulated_data, event_type == 1)$default_time
systemic      <- subset(simulated_data, event_type == 2)$default_time
...
fit_firm     <- fit_dist(firm_specific)
fit_systemic <- fit_dist(systemic)

# best fits
cat("Firm-specific best fit:", fit_firm$best_fit, "\n")
cat("Systemic best fit:", fit_systemic$best_fit, "\n")
```


```{r}
# parameter values for the best fits
cat("Firm-specific gamma fit: shape =", fit_firm$parameters["shape"], 
    " rate =", fit_firm$parameters["rate"], "\n")
cat("Systemic gamma fit: shape =", fit_systemic$parameters["shape"], 
    " rate =", fit_systemic$parameters["rate"], "\n")
```
...
Q-Q plots comparing the empirical data to the fitted gamma marginals support this result visually. Both firm-specific and systemic risks follow the gamma quantiles closely, with minor deviations in the upper tail.

```{r}
# Q-Q plots
par(mfrow = c(1, 2))
qqplot(qgamma(ppoints(length(firm_specific)), 
              shape = fit_firm$parameters["shape"], 
              rate  = fit_firm$parameters["rate"]),
       firm_specific,
       main = "Q-Q Plot: Firm-specific (Gamma)",
       xlab = "Theoretical Quantiles", ylab = "Sample Quantiles")
abline(0, 1, col = "red")

qqplot(qgamma(ppoints(length(systemic)), 
              shape = fit_systemic$parameters["shape"], 
              rate  = fit_systemic$parameters["rate"]),
       systemic,
       main = "Q-Q Plot: Systemic (Gamma)",
       xlab = "Theoretical Quantiles", ylab = "Sample Quantiles")
abline(0, 1, col = "red")
```
# Estimate Parameters

The Frank copula was chosen for this stage of the model because it captures symmetric dependence between risks, meaning that association between failure times is modelled consistently across the entire distribution, rather than being concentrated in the tails. This makes it particularly appropriate for competing risks scenarios where the interaction between failure types is not limited to early or extreme events.
...
We generated 10,000 observations representing time-to-event data for two competing risks: **firm-specific failure** and **systemic failure**. Dependence between the two failure types was introduced using a **Frank copula** with a known parameter of **θ = 2**, chosen for its ability to model **symmetric dependence** — appropriate for scenarios where shared risk factors influence both causes consistently over time, without concentrating in the tails.


The marginal distributions for both risks were set to **gamma**, with $\alpha_1 = 2, \beta_1 = 4$ for firm-specific failure and $\alpha_2 = 2, \beta_2 = 4$ for systemic failure. Observed default times were the minimum of the two, with the cause recorded as the corresponding event type.
...
-   **Histogram of Default Times**: Failures from both causes were distributed relatively evenly over time, consistent with the identical gamma marginals.
...
-   **MLE Estimates** showed a consistent pattern of underestimating the shape parameters and $\theta$, with slight overestimation of the rate parameters. This reflects moderate bias, particularly in the copula component.
...
We simulate two dependent failure times (T1, T2) representing firm-specific and systemic risks using a Clayton copula to induce positive dependence.
...
# Observed failure time and cause
default_time <- pmin(T1, T2)
event_type <- ifelse(T1 < T2, 1, 2)  # 1 = firm-specific, 2 = systemic
...
### Visualize Default Times by Event Type

A histogram of default times colored by event type shows the distribution of observed failures from firm-specific and systemic causes.

```{r default-time-hist}
ggplot(simulated_data, aes(x = default_time, fill = factor(event_type))) +
  geom_histogram(position = "identity", bins = 50, alpha = 0.6) +
  labs(title = "Histogram of Default Times", x = "Time", y = "Count", fill = "Event Type") +
  scale_fill_manual(values = c("steelblue", "tomato"),
                    labels = c("Firm-specific", "Systemic"))
```
...
# Firm-specific Q-Q plot
qqplot(qexp(ppoints(length(firm_specific)), rate = fit_firm$parameters["rate"]),
       firm_specific,
       main = "Q-Q Plot: Firm-specific",
       xlab = "Theoretical Quantiles", ylab = "Sample Quantiles")
abline(0, 1, col = "red")

# Systemic Q-Q plot
qqplot(qexp(ppoints(length(systemic)), rate = fit_systemic$parameters["rate"]),
       systemic,
       main = "Q-Q Plot: Systemic",
       xlab = "Theoretical Quantiles", ylab = "Sample Quantiles")
abline(0, 1, col = "red")
```
...
We generated 10,000 observations representing time-to-event data for two competing risks: **firm-specific failure** and **systemic failure**. Dependence between the two failure types was introduced using a **Clayton copula** with a known parameter of **θ = 2**, chosen for its ability to model **lower tail dependence** — appropriate for scenarios where early failure in one cause increases the risk of failure in another.

The marginal distributions for both risks were set to **exponential**, with **λ₁ = 4** for firm-specific failure and **λ₂ = 2.5** for systemic failure. Observed default times were the minimum of the two, with the cause recorded as the corresponding event type.
...
-   **Histogram of Default Times**: Firm-specific failures dominated early in the timeline, while systemic events occurred more evenly across time, consistent with their lower hazard rate.
